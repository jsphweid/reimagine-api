...
describe("synthesizeSegments", () => {
  it("should make basic array", () => {
    ...
    const expectedOutput = new Float32Array([
      0, 0, 2, 9, 1, 11, 11, 3, 1, 2, 3, 1, 3, 5, 0, 0, 0, 0, 0, 0, 1,
    ]);
  ...
  it("should make small arr", () => {
    const input = [
      { startIndex: 2, arr: [1, 1] },
      { startIndex: 4, arr: [2, 2, 2] },
    ];
    expect(synthesizeSegments(input)).toEqual(
      new Float32Array([0, 0, 1, 1, 2, 2, 2])
    );
  });

  it("should make small arr", () => {
    const input = [
      { startIndex: 0, arr: [1, 1] },
      { startIndex: 1, arr: [2, 2, 2] },
    ];
