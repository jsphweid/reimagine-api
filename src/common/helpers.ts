...
export function synthesizeSegments(segmentData: SegmentDataType[]): Float32Array {
	const maxLength = Math.max(...segmentData.map(segment => segment.arr.length + segment.startIndex))
