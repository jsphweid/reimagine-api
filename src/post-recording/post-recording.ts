...
async function postRecording(base64Blob: any, segmentId: string, samplingRate: number): Promise<{ id: string; error?: string }> {
	try {
		...
		if (!selectResult.rows || !selectResult.rows.length) throw `Couldn't find segment in database.`
