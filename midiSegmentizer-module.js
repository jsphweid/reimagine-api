...
const midiconvert_1 = __webpack_require__(0);
const notes_processor_1 = __webpack_require__(2);
const constants_1 = __webpack_require__(4);
const midiconvert_2 = __webpack_require__(0);
exports.getValidTracks = (midi) => {
    return midi.tracks.filter((track) => track.notes.length > 0 && track.channelNumber >= 0);
};
exports.getSimplePartitioningArray = (arr) => {
    return arr
        .map((num, index) => (index === 0 || num !== 0 ? index : 0))
        .filter((num, index) => index === 0 || num !== 0);
};
exports.getPartitioningArrayWithMax = (arr, max) => {
    const dividingIndicies = [];
    let sum = 0;
...
exports.sliceAndDice = (arr, indicesToSliceAt) => {
    let ret = [];
    for (let i = 0; i < indicesToSliceAt.length; i++) {
        const partitionIndex = indicesToSliceAt[i];
        const nextPartitionIndex = indicesToSliceAt[i + 1];
        ret.push(arr.slice(partitionIndex, nextPartitionIndex));
    }
    return ret;
};
exports.determineMeasureLength = (bpm, timeSignature) => {
    const secondsPerBeat = 1 / (bpm / 60);
    return timeSignature[0] * secondsPerBeat;
};
exports.base64ToBinary = (base64String) => Buffer.from(base64String, 'base64').toString('binary');
exports.processMidiFile = (midiFile) => {
    const midi = midiconvert_1.parse(exports.base64ToBinary(midiFile));
    const { timeSignature, bpm } = midi.header;
    const tracks = exports.getValidTracks(midi);
    const measureLengthInSeconds = exports.determineMeasureLength(bpm, timeSignature);
    const segmentInfos = [];
    // segmentize
    tracks.forEach((track) => {
        const notesGroupedOnRests = notes_processor_1.default.groupNotesOnRests(track.notes);
        const finalDivisons = notes_processor_1.default.subdivideUnderMaxBreath(notesGroupedOnRests, constants_1.MAX_BREATH_SECONDS);
        finalDivisons.forEach((notes) => {
            ...
            const midiJson = midiconvert_2.create();
...
NotesProcessor.getTimeDifferenceArray = (notes) => {
    const diffArray = [0];
...
NotesProcessor.groupNotesOnRests = (notes) => {
    const diffArray = NotesProcessor.getTimeDifferenceArray(notes);
    const partitioningArray = helpers_1.getSimplePartitioningArray(diffArray);
    return helpers_1.sliceAndDice(notes, partitioningArray);
};
NotesProcessor.subdivideUnderMaxBreath = (noteGroupings, maxBreathSeconds) => {
    let finalNotes = [];
    noteGroupings.forEach((notes) => {
        const lengthsOfEachNote = notes.map((note) => note.duration);
        const recommendedSubdivisionIndicies = helpers_1.getPartitioningArrayWithMax(lengthsOfEachNote, maxBreathSeconds);
        finalNotes = [...finalNotes, ...helpers_1.sliceAndDice(notes, recommendedSubdivisionIndicies)];
    });
    return finalNotes;
};
exports.default = NotesProcessor;
...
class ArgumentValidator {
    static hasValidArguments(midiFileBuffer) {
        return (ArgumentValidator.isValidMidiFile(midiFileBuffer) &&
            !ArgumentValidator.isPolyphonicMidiFile(midiFileBuffer) &&
            ArgumentValidator.isSimpleTimeSignature(midiFileBuffer));
    }
    static isValidMidiFile(midiFileBuffer) {
        try {
            midiconvert_1.parse(helpers_1.base64ToBinary(midiFileBuffer));
            return true;
        }
        catch (e) {
            console.error(`
    ...
    static isPolyphonicMidiFile(midiFileBuffer) {
        const midi = midiconvert_1.parse(helpers_1.base64ToBinary(midiFileBuffer));
        const containsPolyphonicTrack = helpers_1.getValidTracks(midi)
            .map((track) => track.notes)
            .map((notes) => ArgumentValidator.noteSequenceIsPolyphonic(notes))
            .filter((trackIsPolyphonic) => trackIsPolyphonic).length > 0;
    ...
    static isSimpleTimeSignature(midiFileBuffer) {
        const midi = midiconvert_1.parse(helpers_1.base64ToBinary(midiFileBuffer));
        return midi.header.timeSignature && midi.header.timeSignature[1] === 4;
    }
}
ArgumentValidator.noteSequenceIsPolyphonic = (notes) => {
    const timeDiffs = notes_processor_1.default.getTimeDifferenceArray(notes);
    return timeDiffs.filter((num) => num < 0).length > 0;
};
exports.default = ArgumentValidator;
...
Object.defineProperty(exports, "__esModule", { value: true });
const program_argument_validator_1 = __webpack_require__(3);
const helpers_1 = __webpack_require__(1);
function segmentizeMidi(midiFile) {
    const hasValidArguments = program_argument_validator_1.default.hasValidArguments(midiFile);
    return hasValidArguments ? helpers_1.processMidiFile(midiFile) : null;
}
exports.segmentizeMidi = segmentizeMidi;
