...
import { pickRandomItem, synthesizeSegments } from './helpers'
import { Context, Callback } from 'aws-lambda'
import { RecordingType, SegmentDataType } from './types'
...
export async function synthesize(event: any, context: Context, callback: Callback) {
	try {
		...
		if (!result.rows && !result.rows.length) throw `couldn't find that pieceId => ${pieceId}`
		...
		return Promise.all(getRequests.map(getRequest => s3.getObject(getRequest).promise()))
			.then(responses => {
				const arrsOfData: SegmentDataType[] = responses.map((response, index) => ({
				...
				const finalBuffer = wav.encode([finalAudioArr], { sampleRate: 44100, float: true, bitDepth: 32 })
